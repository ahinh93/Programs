...
public class AutoCompleteTextField extends JTextField 
{
	...
	private Mode mode;
	public AutoCompleteTextField(int length)
	{
		super(length);
	...
	private class TypeListener implements DocumentListener
	{
		...
		public void insertUpdate(DocumentEvent ev) 
		{
	        ...
	        int w;
	        for (w = pos; w >= 0; w--) {
	            if (! Character.isLetter(content.charAt(w)) && !Character.isWhitespace(content.charAt(w))) {
	                break;
	            }
	        }
	        if (pos - w < 2) {
	        ...
	        if (words.size()>0) 
	        {
	            ...
	            if (match.startsWith(str) || match.toLowerCase().startsWith(str)) 
	           	{	
	 ...
	 private class CompletionTask implements Runnable 
	 {
	        ...
	        public void run() 
	        {
	            ...
	            AutoCompleteTextField.this.setText(AutoCompleteTextField.this.getText().substring(0, 1).toUpperCase() + 
	            		AutoCompleteTextField.this.getText().substring(1)+completion);
